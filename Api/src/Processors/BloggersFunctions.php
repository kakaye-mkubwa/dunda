...
//                }
    ...
    public function addPost($postTitle, $postDescription, $postContent, $authorID,$categoryID, $imageTempName, $imageFileName, $baseDir){
        ...
        $query = "INSERT INTO blog_posts(postTitle,postDescription,postContent,authorID,categoryID,postImage) VALUES(?,?,?,?,?,?)";
        ...
        try{
            if (move_uploaded_file($imageTempName,$imageFilePath)){
                if ($stmt = mysqli_prepare($conn,$query)){
                    mysqli_stmt_bind_param($stmt,'sssiis',$paramPostTitle,$paramPostDescription,$paramPostContent,$paramAuthorID,$paramCategoryID,$paramPostImage);
        ...
        if ($this->verifyPassword($bloggerID,$inputPassword)){
            if ($stmt = mysqli_prepare($conn,$query)){
                ...
                if (mysqli_stmt_execute($stmt)){
                    ...
                    $output = array("error"=>"false","message"=>"Password changing failed");
                }
                mysqli_stmt_close($stmt);
            }else{
                $this->log->error("Prepare failed ".mysqli_error($conn));
        ...
        if ($stmt = mysqli_prepare($conn,$query)){
            ...
            if (mysqli_stmt_execute($stmt)){
                ...
                if (mysqli_stmt_affected_rows($stmt) == 1){
                    ...
                    $this->log->error("Failed verifying password : more than one rows fetched $bloggerID");
                    $output = false;
                }
                mysqli_stmt_close($stmt);
            }else{
    ...
    public function fetchBloggerDetails($bloggerID){
        ...
        $query = "SELECT username,imageUrl,description FROM bloggers WHERE memberID = ?";
        ...
        if ($stmt = mysqli_prepare($conn,$query)){
            ...
            if (mysqli_stmt_execute($stmt)){
                mysqli_stmt_bind_result($stmt,$fetchedUsername,$fetchedImageUrl,$fetchedDescription);
                while(mysqli_stmt_fetch($stmt)){
//                    $imageURL = "https://top10betz.com".substr($fetchedImageUrl,26);
                    $imageURL = "../".substr($fetchedImageUrl,51);

                    $data[] = array("username"=>$fetchedUsername,"description"=>$fetchedDescription,"image_url"=>$imageURL);
                }
    ...
    public function fetchBloggerDetailsMain($bloggerID){
        $dbConnect = new DBConnect();
        $conn = $dbConnect->dbConnection();
        ...
        if ($stmt = mysqli_prepare($conn,$query)){
            ...
            if (mysqli_stmt_execute($stmt)){
                ...
                while(mysqli_stmt_fetch($stmt)){
//                    $imageURL = "https://top10betz.com".substr($fetchedImageUrl,26);
                    $imageURL = substr($fetchedImageUrl,52);
